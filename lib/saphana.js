...
SAPHANA.prototype.create = function (model, data, callback) {
    ...
    self.query(sql, fieldsAndData.params, function (err, results) {
        ...
        var modelPKID = self.idName(model);
...
SAPHANA.prototype.toDatabase = function (prop, val) {
    ...
    return this.escapeValue(val.toString());
};
...
/**
 * Escape value for SAP HANA
 *
 * @param {String} value The value
 * @returns {String} The escaped value
 */
SAPHANA.prototype.escapeValue = function (value) {
    if (!value) {
        return value;
    }
                ...
                if (filterValue === 'NULL') {
                    ...
                    switch (condType) {
                        ...
                        case 'nlike':
                            ...
                            break;
                        case 'ilike':
                            sqlCond = 'LOWER(' + sqlCond + ') LIKE ';
                            filterValue = 'LOWER(' + filterValue + ")ESCAPE '\\'";
                            break;
                        case 'nilike':
                            sqlCond = 'LOWER(' + sqlCond + ') NOT LIKE ';
                            filterValue = 'LOWER(' + filterValue + ")ESCAPE '\\'";
                            break;
                        default:
                            sqlCond += ' ' + condType + ' ';
...
/**
 * Build the SQL clause with schema and table name.
 * @param {String} model The model name
 * @param {Object} filter The filter
 * @returns {*}
 */
SAPHANA.prototype.toFilter = function (model, filter) {
    ...
    return self.tableEscaped(model) + ' ' + self.toFilterConds(model, filter);
};
 ...
 * @returns {*}
 */
SAPHANA.prototype.toFilterConds = function (model, filter) {
    var self = this;
    var out = '';
